...
5. Copy seed: `pack/seed/*` → `coordinator-data/settings/` (locale, `coordinator.json` with empty name, empty team, empty project_profile) and `current_author.example`. Do **not** ask for project name, coordinator name, alias, or people in chat — the first setup screen confirms that. Do **not** set `setup.completed`.
