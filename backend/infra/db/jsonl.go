...
INSERT INTO events(timestamp, event, task_id, branch, alias, repo, service, status, cost_usd, budget_usd, ondemand_usd, cursor_models_pct, other_models_pct, usage_plan, plan_price_usd, spend_kind, summary, findings, active_seconds, activity_windows, source_file, source_line)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
		return err
	}
	...
	for scanner.Scan() {
		...
		if _, err := insert.ExecContext(ctx, ev.Timestamp, ev.Event, ev.TaskID, ev.Branch, ev.Alias, ev.Repo, ev.Service, ev.Status,
			floatPtrValue(ev.CostUSD), floatPtrValue(ev.BudgetUSD), floatPtrValue(ev.OnDemandUSD), floatPtrValue(ev.CursorModelsPct), floatPtrValue(ev.OtherModelsPct), ev.UsagePlan, floatPtrValue(ev.PlanPriceUSD), ev.SpendKind, ev.Summary, ev.Findings, int64PtrValue(ev.ActiveSeconds), marshalActivityWindows(ev.ActivityWindows),
